...
float fbm2(vec2 uv, float t){
  vec3 p=vec3(uv*uScale,t); float amp=1.0,freq=1.0,sum=1.0;
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    const needsReinitKeys: Array<keyof ReinitConfig> = ['antialias', 'liquid', 'noiseAmount']
    ...
    if (mustReinit) {
      ...
      const timeOffset = (u32[0] / 0xffffffff) * 1000
      ...
      if (liquid) {
        ...
        composer.addPass(new RenderPass(scene, camera)); composer.addPass(effectPass)
      }
      if (noiseAmount > 0) {
        if (!composer) { composer = new EffectComposer(renderer); composer.addPass(new RenderPass(scene, camera)) }
        const noiseEffect = new Effect('NoiseEffect', `uniform float uTime;uniform float uAmount;float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}void mainUv(inout vec2 uv){}void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float n=hash(floor(uv*vec2(1920,1080))+floor(uTime*60.0));float g=(n-0.5)*uAmount;outputColor=inputColor+vec4(vec3(g),0.0);}`, { uniforms: new Map([['uTime', new THREE.Uniform(0)], ['uAmount', new THREE.Uniform(noiseAmount)]]) })
        const noisePass = new EffectPass(camera, noiseEffect); noisePass.renderToScreen = true
        composer.passes.forEach(p => (p.renderToScreen = false)); composer.addPass(noisePass)
      }
      if (composer) composer.setSize(renderer.domElement.width, renderer.domElement.height)

      const mapToPixels = (e: PointerEvent) => {
        const rect = renderer.domElement.getBoundingClientRect()
        const scaleX = renderer.domElement.width / rect.width, scaleY = renderer.domElement.height / rect.height
        return { fx: (e.clientX - rect.left) * scaleX, fy: (rect.height - (e.clientY - rect.top)) * scaleY, w: renderer.domElement.width, h: renderer.domElement.height }
      }
    ...
    return () => {
      ...
      t.quad?.geometry.dispose(); t.material.dispose(); t.composer?.dispose()
      t.renderer.dispose(); t.renderer.forceContextLoss()
      if (t.renderer.domElement.parentElement === container) container.removeChild(t.renderer.domElement)
      threeRef.current = null
    }
